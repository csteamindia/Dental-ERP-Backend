        ...
        <table border="1">
          ...
          <tbody>
                ...
                foreach($pros as $p){
                  if($p->order_id == $i->order_id){
                    ...
                    <td>&nbsp;'.$i->patiant.'</td>
                      ...
                      <td>&nbsp;'.$p->unit.'</td>
                      <td>&nbsp;<b>'.number_format($p->unitprice).'</b></td>
                      <td>&nbsp;<b>'.number_format(str_replace(',', '', $p->subtotal)).'</b></td>
                    </tr>';
                    $tunits += $p->unit;
                  }
                }
            ...
            <tr>
              ...
              <td>&nbsp;Sub Total &nbsp; </td>
              <th>&nbsp;<?= number_format($i->invoice_subtotal);?></th>
            </tr>
            <tr>
              ...
              <th colspan='1'>
                <table border="1" width="100%">
                  ...
                  <?php if($i->invoice_gst_amount > 0) {?>
                    <tr>
                      <td>IGST@<?= number_format($i->igst);?></td>
                    </tr>
                    <tr>
                      <td>CGST@<?= number_format($i->cgst);?></td>
                    </tr>
                    <tr>
                      <td>SGST@<?= number_format($i->sgst);?></td>
                    </tr>
                  <?php } ?>
              ...
              <td colspan='3'>
                <table border="1" width="100%">
                  <tr><th colspan="3">&nbsp;<?= number_format(($i->discount));?>/-</th></tr>
                  <?php if($i->invoice_gst_amount > 0) {?>
                    <tr>
                      <td colspan="3">&nbsp;<?= number_format((($i->invoice_subtotal*$i->igst)/100));?>/-</td>
                    </tr>
                    <tr>
                      <td colspan="3">&nbsp;<?= number_format((($i->invoice_subtotal*$i->cgst)/100));?>/-</td>
                    </tr>
                    <tr>
                      <td colspan="3">&nbsp;<?= number_format((($i->invoice_subtotal*$i->sgst)/100));?>/-</td>
                    </tr>
                  <?php } ?>
                  <tr><th colspan="3">&nbsp;<?= number_format(($i->invoice_total));?>/-</th></tr>
                </table>
              </td>
            </tr>
          </tbody>
